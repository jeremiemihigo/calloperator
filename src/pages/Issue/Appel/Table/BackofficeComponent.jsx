...
function BackOfficeComponent({ data, analyse }) {
  const { setPlainteSelect, setSelect } = React.useContext(CreateContexteTable);
  const plainte = [
    "Activation & rafraichissement Canal +",
    "Rafraichissement des chaines canal+",
  ];
  ...
  const handleChanges = () => {
    setFilterFn({
      fn: () => {
        return data.filter(
          (x) =>
            plainte.includes(x.plainteSelect) || plainte.includes(x.typePlainte)
        );
      },
    });
  };
  return (
    <>
      ...
      {data && data.length === 0 && (
        <NoCustomer texte="No backoffice complaints" />
      )}
    </>
  );
}

export default React.memo(BackOfficeComponent);
