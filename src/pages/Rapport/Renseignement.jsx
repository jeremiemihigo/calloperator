...
function Renseignement() {
  ...
  return (
    <div>
      <Paper sx={{ padding: '10px', marginBottom: '10px', alignItems: 'center', justifyContent: 'space-between', display: 'flex' }}>
        <Typography sx={{ textAlign: 'center' }}>All inquiries recorded at the shop and call center</Typography>
        {data && data.length > 0 && (
          <div style={{ width: '20%' }}>
            <ExcelButton data={data} title="Export in Excel" fileName="Toutes_les_renseignements.xlsx" />
          </div>
        )}
      </Paper>
      {load && <LoaderGif width={300} height={300} />}
      {!load && data && data.length === 0 && <NoCustomer texte="No information found" />}
      {data && data.length > 0 && (
        <Paper elevation={2}>
          <DataGrid
            rows={data}
