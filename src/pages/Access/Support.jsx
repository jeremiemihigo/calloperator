...
import { Button, FormControl, InputAdornment, OutlinedInput, Typography } from '@mui/material';
...
function Support() {
  ...
  const addone = () => {
    dispatch(OtherUpdated({ data: { taches: '31660' }, unset: {}, idAgent: agentSelect._id }));
    setAgentSelect('');
  };
  ...
  function AddTache() {
    return (
      <div style={{ width: '20rem' }}>
        <AutoComplement propr="nom" options={agent} setValue={setAgentSelect} value={agentSelect} />;
        <div>
          <Button onClick={() => addone()} variant="contained" color="primary" fullWidth>
            Valider
          </Button>
        </div>
      </div>
  ...
  return (
    <div>
      ...
      >
        Ajoutez un agent dans la team support
      </Typography>
      ...
      {agent &&
        agent.length > 0 &&
        filterFn.fn(agent.filter((x) => x.taches && x.taches.length > 0)).map((item) => {
          return (
            <Chip
              sx={{ margin: '2px' }}
              key={item._id}
