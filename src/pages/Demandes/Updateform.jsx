...
function UpdateForm({ update, show }) {
  ...
  const modifier = async () => {
    ...
    try {
      if (
        user?.nom !== update?.agentSave?.nom &&
        user?.fonction !== "superUser"
      ) {
        setMessage(
          `seulement ${update?.agentSave?.nom} peut apporter une modification à cette réponse`
        );
        setOpenSnack(true);
        setSending(false);
      } else {
  ...
  React.useEffect(() => {
    if (update) {
      ...
      let adresse = optionChange.filter(
        (x) => x.value === update?.adresschange
      );
