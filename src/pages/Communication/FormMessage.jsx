...
function FormMessage({ dataToUpdate, setDataToUpdate }) {
  ...
  const sendData = async (e) => {
    try {
      e.preventDefault();
      dispatch(Ajoutercommunication({ title, content, date, concerne: fonction?.value }));
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const sendEdit = (e) => {
    e.preventDefault();
    try {
      dispatch(UpdateCommunication({ id: dataToUpdate._id, data: { title, content, date, concerne: fonction?.value } }));
      annuler();
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <div>
      <p style={{ textAlign: 'center', padding: 0, margin: 0, fontSize: '10px' }}>
        @name: pour afficher le nom de l&apos;agent dans le message
      </p>
      <p style={{ textAlign: 'center', fontSize: '10px' }}>@p: pour un paragraphe</p>
      <div className="mb-3">
      ...
      <Grid sx={{ marginTop: '10px' }} container>
        <Grid item lg={6}>
          {' '}
          <Button onClick={dataToUpdate ? (e) => sendEdit(e) : (e) => sendData(e)} fullWidth color="primary" variant="contained">
            {dataToUpdate ? (
        ...
        <Grid item lg={6} sx={{ paddingLeft: '5px' }}>
          {' '}
          {dataToUpdate && (
            <Button onClick={() => annuler()} fullWidth color="warning" variant="contained">
