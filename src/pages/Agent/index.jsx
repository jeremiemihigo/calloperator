...
function AgentListe() {
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <span
            style={{
              color: `${params.row.active ? "blue" : "red"}`,
              fontWeight: "bolder",
            }}
          >
            {params.row.fonction}
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <span>{params.row.first ? params.row.pass : "Just edited"}</span>
        );
      },
    },
  ...
  return (
    <Paper elevation={3} sx={{ padding: "5px" }}>
      {allListe.reinitialiser === "success" && agentReset && (
        <DirectionSnackbar
          message={`Reinitialisation effectuée ${agentReset}`}
          open={open}
          setOpen={setOpen}
        />
      )}
      ...
      <Popup
        open={openAgentUpdate}
        setOpen={setOpenAgentUpdate}
