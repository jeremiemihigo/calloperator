...
function SelectForm() {
  ...
  const { setLoadQuestion, formSelect, setFormSelect, setData } =
    React.useContext(ContextParametre);

  const loading = async () => {
    try {
      setLoadQuestion(true);
      const response = await axios.get(
        portofolio + "/readQuestionFormular/" + formSelect?.idFormulaire,
        config
      );
