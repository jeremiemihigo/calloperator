...
function Affichage() {
  const { dataexcel, setDataExcel } = React.useContext(CreateContextePerformance);
  const columns = [
    {
      field: 'id',
      headerName: '#',
    ...
    },
    {
      ...
      renderCell: (p) => {
        return (
          <>
            <Delete onClick={() => setDataExcel(dataexcel.filter((x) => x.id !== p.row.id))} sx={{ cursor: 'pointer' }} fontSize="small" />
          </>
        );
      }
    }
