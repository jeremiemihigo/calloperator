...
function UploadClient() {
  ...
  const uploadCustomer = async (e) => {
    ...
    setSending(true);
    try {
      const response = await axios.post(lien_dt + '/changebyfile', { data }, config);
    ...
    <>
      ...
      {data && !sending && data.filter((x) => !x.status).length === 0 && (
        <Typography
          ...
          sx={{ fontSize: '12px', cursor: 'pointer', fontWeight: 'bolder', marginTop: '10px', textAlign: 'right', color: 'blue' }}
        >
          Envoyer
        </Typography>
      )}
